...
public export
rotation : (n : Node) -> Rot (shape n)
rotation (MkNode id shape conn rot spaces) = rot
...
public export
rotToList : (shape : Shape) -> Rot shape -> List NodeId
rotToList Free _ = []
rotToList Leaf xs = toList xs
rotToList Edge xs = toList xs
rotToList Sat xs = toList xs
